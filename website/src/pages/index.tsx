import func from "@root/shared/lib/helpers/func";
import axios from "axios";
import { wrapper } from "@app/app/store/store";
import {
  API_URL,
  HOME_FIRST_BOX_CATEGORY_ID,
  HOME_OFFER_LIST_ID,
  HOME_SLIDER_CATEGORY_ID,
} from "@root/config";
import type { GetServerSideProps, NextPage } from "next";
import type { SliderItem } from "@app/widgets/home/HomeSlider";

import dynamic from "next/dynamic";

import "swiper/css";
import "swiper/css/navigation";
import "swiper/css/thumbs";

const HomeFirstBox = dynamic(
  () => import("@app/widgets/home/HomeFirstBox"),
);
const HomeSeccoundBoxs = dynamic(
  () => import("@app/widgets/home/HomeSeccoundBoxs"),
);
const HomeOfferList = dynamic(
  () => import("@app/widgets/home/HomeOfferList"),
);
const HomeProductsFirst = dynamic(
  () => import("@app/widgets/home/HomeProductsFirst"),
);
const Brands = dynamic(() => import("@app/entities/brands/ui"));
const HomeSlider = dynamic(() => import("@app/widgets/home/HomeSlider"));
const Head = dynamic(() => import("@app/shared/ui/SeoHead"));

type CategoryItem = {
  _id: string;
  title?: string;
  description?: string;
  [key: string]: unknown;
};

type ProductItem = {
  _id: string;
  seo: string;
  title: string;
  [key: string]: unknown;
};

type SliderNode = SliderItem & {
  children?: SliderNode[];
};

type HomeBoxItem = {
  _id: string;
  link: string;
  image: string;
  title: string;
};

type HomeBoxNode = HomeBoxItem & {
  children?: HomeBoxNode[];
};

type OfferItem = {
  _id: string;
  link: string;
  image: string;
  title: string;
  description?: string;
};

type OfferNode = OfferItem & {
  children?: OfferNode[];
};

interface HomeProps {
  resData: CategoryItem[];
  resProductFirst: ProductItem[];
  resProductSeccond: ProductItem[];
}

const HomePage: NextPage<HomeProps> = ({
  resData = [],
  resProductFirst = [],
  resProductSeccond = [],
}) => {
  const homeSliderUnknown = func.getCategoriesTree(
    resData,
    HOME_SLIDER_CATEGORY_ID,
  );
  const homeSlider: SliderItem[] = Array.isArray(homeSliderUnknown)
    ? (homeSliderUnknown as SliderNode[]).map(({ children, ...rest }) => rest)
    : [];
  const homeFirstBoxUnknown = func.getCategoriesTree(
    resData,
    HOME_FIRST_BOX_CATEGORY_ID,
  );
  const homeFirstBox: HomeBoxItem[] = Array.isArray(homeFirstBoxUnknown)
    ? (homeFirstBoxUnknown as HomeBoxNode[]).map(({ children, ...rest }) => rest)
    : [];
  const homeOfferListUnknown = func.getCategoriesTree(
    resData,
    HOME_OFFER_LIST_ID,
  );
  const homeOfferList: OfferItem[] = Array.isArray(homeOfferListUnknown)
    ? (homeOfferListUnknown as OfferNode[]).map(({ children, ...rest }) => rest)
    : [];

  const offerItem = resData.find((val) => val._id === HOME_OFFER_LIST_ID);
  const homeOfferListtitle = {
    title: offerItem?.title,
    description: offerItem?.description,
  };

  return (
    <div>
      <Head />
      <Brands />
      <HomeSlider state={homeSlider} />
      <HomeProductsFirst
        state={resProductFirst}
        title={{
          title: "Best Sellers",
          description: "Our Most Popular Products",
        }}
      />
      <HomeFirstBox state={homeFirstBox} />
      <HomeSeccoundBoxs
        state={resProductSeccond}
        title={{
          title: "New Products",
          description: "We Added New Products For You",
        }}
      />
      <HomeOfferList state={homeOfferList} title={homeOfferListtitle} />
    </div>
  );
};

export const getServerSideProps: GetServerSideProps<HomeProps> =
  wrapper.getServerSideProps(() => async () => {
    const response = await axios.get(`${API_URL}/homesliderpublic`);

    const filterObjectFirst = {
      sort: { saleqty: -1 },
      limit: 10,
      skip: 0,
    };

    const responseProductFirs = await axios.post(
      `${API_URL}/productspublic/home`,
      filterObjectFirst,
    );

    const filterObjectSeccond = {
      sort: { createdAt: -1 },
      limit: 15,
      skip: 0,
    };

    const responseProductSeccond = await axios.post(
      `${API_URL}/productspublic/home`,
      filterObjectSeccond,
    );

    return {
      props: {
        resData: response.data,
        resProductFirst: responseProductFirs.data,
        resProductSeccond: responseProductSeccond.data,
      },
    };
  });

export default HomePage;
