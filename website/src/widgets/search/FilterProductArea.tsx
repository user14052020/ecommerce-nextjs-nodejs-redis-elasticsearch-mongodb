...
import ProductCard from "@app/entities/product/ui/ProductCard";
...
const FilterProductArea = () => {
   ...
   const getProducts = () => {
      ...
      axios
         ...
         .then((res) => {
            const searchSource = res.headers?.["x-search-source"];
            const searchError = res.headers?.["x-search-error"];
            if (searchSource === "mongo") {
               console.info(
                  "[search] source=mongo",
                  searchError ? `elastic_error=${searchError}` : ""
               );
            }
