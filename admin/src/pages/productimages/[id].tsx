...
import type {
  ...
  ProductRef,
  SelectOption,
} from "@app/entities/productimages/model/types";
...
const Default: NextPage<ProductImagesEditProps> = ({
  ...
  const productId =
    typeof state.product_id === "string"
      ? state.product_id
      : state.product_id?._id ?? undefined;
  const fields: FieldData[] = Object.entries(state).map(([name, value]) => ({
    name,
    value: name === "product_id" ? productId : value,
  }));
    ...
    axios
      ...
      .then((res) => {
        if (res.data.variant == "error") {
          ...
          if (productId) {
            Router.push("/productimages/list?id=" + productId);
          } else {
