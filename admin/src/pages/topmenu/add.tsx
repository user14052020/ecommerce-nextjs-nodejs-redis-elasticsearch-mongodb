...
const Default: NextPage<TopMenuAddProps> = ({ getCategories = [] }) => {
  const Editor = dynamic(() => import("@app/shared/ui/Editor")) as (
  ...
  const fields: FormFieldData[] = Object.entries(state).map(([name, value]) => ({
    name,
    value,
  }));
  const [dataCategories, setDataCategories] = useState<TopMenuTreeOption[]>([
    ...
    axios
      ...
      .then((res) => {
        if (res.data.length > 0) {
          const data = func.getCategoriesTreeOptions(res.data) as TopMenuTreeOption[];
...
Default.getInitialProps = async ({ req }: NextPageContext) => {
  if (!req?.headers?.cookie) {
    ...
    const getDataCategoriesManipulate: TopMenuTreeOption[] = [
