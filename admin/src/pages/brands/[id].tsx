...
import type {
  ...
  BrandFormValues,
  BrandItem,
} from "@app/entities/brand/model/types";
import type { FieldData } from "@app/shared/types/form";

const Default: NextPage<BrandEditProps> = ({ getData }) => {
  const intl = useIntl();
  const [state, setState] = useState<BrandItem>(getData || ({} as BrandItem));
  ...
  function getDataFc(brandId: string) {
    axios.get<BrandItem>(`${API_URL}/brands/${brandId}`).then((response) => {
  ...
  const onSubmit = async (Data: BrandFormValues) => {
    ...
    axios
      .post(`${API_URL}/brands/${id}`, payload)
      .then((res) => {
