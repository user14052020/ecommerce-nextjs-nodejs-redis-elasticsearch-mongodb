...
import IntlMessages from "@root/shared/ui/IntlMessages";
...
import type {
  ...
  TurkeyMahalle,
  TurkeySemt,
} from "@app/entities/customer/model/types";
import type { FieldData } from "@app/shared/types/form";

const Default: NextPage<CustomersEditProps> = ({
  dataCityOption = [],
  dataCity = [],
  getData = {},
}) => {
  ...
  const [state, setState] = useState<CustomerData>(getData);
  ...
  function getDataFc() {
    ...
    axios.get(`${API_URL}/customers/${customerId}`).then((response) => {
      const data = response.data as CustomerData;
      data["password"] = "";
      setState(data);
  ...
  const onSubmit = (Data: CustomerFormValues) => {
    ...
    axios
      .post(`${API_URL}/customers/${customerId}`, Data)
      .then((res) => {
...
Default.getInitialProps = async ({ req, query }: NextPageContext) => {
  if (!req?.headers?.cookie) {
    ...
    const dataManipulate = getDataCity.data.map((item: TurkeyCity) => ({
      label: item.Il,
      value: item.Il,
    }));
